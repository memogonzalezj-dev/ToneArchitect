...
import { Cpu, Download, CheckCircle2, AlertCircle, RefreshCw, ExternalLink, HardDrive, MemoryStick, Zap } from "lucide-react";
import {
  checkOllamaRunning,
  checkModelAvailable,
  pullModel,
...
type Step = "checking" | "need_ollama" | "need_model" | "pulling" | "ready";

interface Props {
  onReady: () => void;
}

export default function OllamaSetup({ onReady }: Props) {
  ...
  const check = useCallback(async () => {
    ...
    if (!running) { setStep("need_ollama"); setChecking(false); return; }
  ...
  const startPull = async () => {
    ...
    try {
      ...
      setPullError(e instanceof Error ? e.message : "Download failed. Check that Ollama is running.");
      setStep("need_model");
    }
  };
            ...
            <AnimatePresence mode="wait">
              ...
              {step === "checking" && (
                <motion.div key="checking" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <span className="text-sm text-white/50">Checking Ollama status…</span>
                </motion.div>
              )}

              {/* Need Ollama installed */}
              {step === "need_ollama" && (
                <motion.div key="need_ollama" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="space-y-6">
                  <div className="flex items-start gap-3">
                    <AlertCircle className="w-5 h-5 text-amber-400 mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-sm font-semibold text-white">Ollama not detected</p>
                      <p className="text-[11px] text-white/40 mt-1 leading-relaxed">
                        ToneArchitect.hx runs AI entirely on your Mac — no cloud, no API keys, no subscription.
                        Install Ollama first (it's free and takes 2 minutes).
                      </p>
                    </div>
                  </div>
                  ...
                  <ol className="space-y-3">
                    {[
                      { n: 1, text: "Download and install Ollama from ollama.com" },
                      { n: 2, text: "Launch Ollama — it appears in your menu bar" },
                      { n: 3, text: 'Click "Check again" below' },
                    ].map((s) => (
                      <li key={s.n} className="flex items-start gap-3 text-[11px] text-white/50">
                        <span className="text-blue-500 font-mono font-bold flex-shrink-0 mt-0.5">{s.n}.</span>
                        <span>{s.text}</span>
                      </li>
                    ))}
                  </ol>
                  ...
                  <div className="flex gap-3">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex-1 h-11 bg-blue-500 hover:bg-blue-400 text-white font-bold tracking-[0.2em] text-[10px] transition-colors flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      onClick={check}
                      disabled={checking}
                      className="h-11 px-5 border border-white/20 text-white/50 hover:text-white hover:border-white/40 text-[10px] font-mono tracking-widest transition-colors flex items-center gap-2 disabled:opacity-40"
                    >
                ...
                <motion.div key="need_model" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-white/40 mt-1 leading-relaxed">
                        One-time download of <span className="font-mono text-white/70">{REQUIRED_MODEL}</span> (~5 GB).
